"""
Read-Only Data Loader for Stage 3 Clinical NLP.
Loads upstream datasets with strict schema validation and split integrity verification.
"""
...
def _validate_schema(df: pd.DataFrame, source_name: str) -> None:
    """Ensure dataframe conforms to the frozen upstream schema."""
    ...
    invalid_urg = set(df["urgency_level"].unique()) - VALID_URGENCY_LEVELS
    if invalid_urg:
        raise ValueError(f"Unrecognized urgency levels {invalid_urg} in {source_name}!")

    invalid_haz = set(df["hazard_type"].unique()) - VALID_HAZARD_TYPES
    if invalid_haz:
        raise ValueError(f"Unrecognized hazard types {invalid_haz} in {source_name}!")


def load_train_data() -> pd.DataFrame:
    """Load the official 70% TRAIN partition in read-only mode."""
...
def load_validation_data() -> pd.DataFrame:
    """Load the official 15% VALIDATION partition in read-only mode."""
    ...
    _validate_schema(df, "VALIDATION")
    return df


def load_locked_test_data() -> pd.DataFrame:
    """
    Load the official 15% LOCKED TEST partition in read-only mode.
    WARNING: The locked test set is strictly isolated. It must NEVER be used for model tuning,
    hyperparameter optimization, feature selection, or engineering threshold choices.
    """
    ...
    _validate_schema(df, "LOCKED_TEST")
    return df
...
def load_full_dataset() -> pd.DataFrame:
    ...
    _validate_schema(df, "CANONICAL_DATASET")
    return df
