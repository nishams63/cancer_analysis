...
- **Audit Lineage**: Complete issue logs are persisted to `stage5/data/interim/cleaned/cleaning_issues_log.json`.
"""
...
**Execution Timestamp**: {VersionManager.get_timestamp()}  
**Compiled Artifact**: `stage5/data/processed/reference_distributions.parquet`  
...
**Execution Timestamp**: {VersionManager.get_timestamp()}  
**Catalog Path**: `stage5/configs/rare_combination_space.yaml`  
...
- **Zero Hallucination Retrieval**: Downstream RAG retrievers are strictly restricted to the approved evidence store in `stage5/data/evidence/chunks/`.
"""
    (reports_dir / "evidence_summary.md").write_text(ev_md, encoding="utf-8")
    logger.info("All 4 Stage 5 reports generated in stage5/reports/")

def run_master_data_engineering(config_path: str = "stage5/configs/stage5_data_config.yaml"):
    logger.info("=" * 70)
    ...
    # 2. Source Registry
    reg = SourceRegistry("stage5/configs/source_registry.yaml")
    ...
    cleaned_dir = Path("stage5/data/interim/cleaned")
    ...
    norm_dir = Path("stage5/data/interim/normalized")
    ...
    generate_reports(
        ...
        reports_dir=Path("stage5/reports")
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--config", default="stage5/configs/stage5_data_config.yaml", help="Path to config YAML")
