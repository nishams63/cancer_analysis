...
import { BrainCircuit, FileText, FlaskConical, GitCompareArrows, Mic, Send, ShieldCheck, Sparkles, TestTube2, Volume2 } from "lucide-react";
...
export function CopilotDock() {
  ...
  return <aside className={`copilot-dock copilot-${ai.emotion}`} aria-label="ONCO.AI Clinical Copilot">
    <header><span className="copilot-logo"><BrainCircuit /></span><div><b>ONCO.AI</b><small>Clinical Copilot</small></div><i /><em>Online</em></header>
    ...
    <section className="copilot-conversation">
      ...
      <div className="copilot-state"><span className="waveform"><i/><i/><i/><i/><i/></span><b>{ai.emotion === "thinking" ? "Retrieving evidence…" : ai.emotion === "speaking" ? "Speaking…" : ai.listening ? "Listening…" : "Ready"}</b>{ai.voiceEnabled && <Volume2 />}</div>
      <div className="copilot-safety"><ShieldCheck /> AI-generated clinical decision support. Physician review required.</div>
    </section>
  </aside>;
}
