...
class DraftTargetGenerator:
    ...
    def synthesize_target(
        ...
        has_active_aes = clean_aes and not any(
            str(ae).lower() in ("no acute adverse toxicities", "no acute toxicities")
            or str(ae).lower().startswith("no ")
            for ae in clean_aes
        )
        ...
        if has_active_aes:
            ...
            findings.append(f"exhibits stable tolerance with {ae_str}")
        else:
